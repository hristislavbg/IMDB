...
import com.imdb.movies.filter.CustomAuthenticationFilter;
import com.imdb.movies.filter.CustomAuthorizationFilter;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests().antMatchers(GET, "/api/v1/favorite/movies").authenticated();
        http.authorizeRequests().antMatchers(POST, "/api/v1/favorite/movies/create/{id}").authenticated();
        http.authorizeRequests().antMatchers(DELETE, "/api/v1/favorite/movies/delete/{id}").authenticated();
        ...
        http.authorizeRequests().antMatchers(DELETE, "/api/v1/movies/delete/{id}").hasAuthority("admin");
        http.authorizeRequests().antMatchers(PUT, "/api/v1/movies/rating/{id}/{rating}").authenticated();
        http.authorizeRequests().antMatchers(POST, "/api/v1/movies/actor/create/{movieId}/{actorId}").hasAuthority("admin");
        http.authorizeRequests().antMatchers(POST, "/api/v1/movies/actor/delete/{movieId}/{actorId}").hasAuthority("admin");
